...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin[range(num_train),y]=0 #every classes' scores compute with lable, except lable's
  
  # loss = margin[margin>0].sum()+reg * torch.sum(W * W)
  ...
  margin[margin > 0] = 1 # count the number of positive margin
  margin[range(num_train), y] = -margin.sum(dim = 1)
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = torch.mm(X, W).max(dim=1).indices
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  regularization_strengths = [1, 10, 11, 16]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    # Then we calcu p=(e^s[k])/(sum(e^s[0~c]))
    # Bad way: Numeric problem, potential blowup
    # scores=torch.exp(scores)/torch.sum(torch.exp(scores))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  # There is A important var
  # scores(before exp)
  

  scores = X.mm(W)
  # scores = scores-scores.max(dim = 1).values.view(-1,1)
  scores = scores-scores.max(dim = 1,keepdim=True).values
  e_scores = scores.exp()
  e_scores_sum = e_scores.sum(dim = 1)

  # loss = ln(sum of e^scores)-scores[right class]
  loss = torch.sum( e_scores_sum.log() - scores[range(num_train), y] )

  loss /= num_train
  loss += reg * torch.sum(W * W)

  # dw
  # part 1: dp/dw= x.T * e^scores/sum_e^scores
  # here is how to divide
  M = ((e_scores/e_scores_sum.view(-1,1)))
  # part 2: dl/dw=x.T * (e^scores/sum_e^scores - 1)
  # here is how to minus 
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = torch.linspace(1e-3, 2e-1, 4) 
  regularization_strengths = torch.linspace(1e-4, 1e-3, 4)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
