...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    # scores = scores - scores.max(1).values.view(-1,1)
    scores -= scores.max(dim=1, keepdim=True).values
    # loss = - ln(prob = e^s/sum_of_e^s) = - (s-sum_of_e^s)
    # obvious  (s-sum_of_e^s) is ln(prob)
    # ln_probs = scores.exp().sum(dim=1).log() - scores[torch.arange(N),y]
    ln_probs = scores - scores.exp().sum(dim=1, keepdim=True).log() 
    loss = (-1.0/ N) * ln_probs[torch.arange(N), y].sum()
    ...
    # dl/dw=x.T * (e^scores/sum_e^scores - 1)

    # dl/dw2= x.T * prob (-1 optional)    
    probs[torch.arange(N), y] -= 1
    probs /= N

    grads['b2'] = probs.sum(dim = 0)
    grads['W2'] = h1.t().mm(probs) + 2* W2 * reg
...
def nn_get_search_params():
  ...
  hidden_sizes = torch.linspace(8,128,4).int()
  regularization_strengths = [1e1,1e0,1e-1,1e-2]
  ...
  for reg in regularization_strengths:
    for lr in learning_rates:
      for hs in hidden_sizes:
        print('train with hidden_size: {}, learning_rate: {}, regularization: {}'.format(hs,lr,reg))
        net = TwoLayerNet(3 * 32 * 32, hs, 10)
        # copy codes from .ipynb
