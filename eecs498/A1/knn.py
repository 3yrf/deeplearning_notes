...
def compute_distances_two_loops(x_train, x_test):
  ...
  for ctest in range(num_test):
    for ctrain in range(num_train):
      dists[ctrain,ctest]=((xtr[ctrain]-xte[ctest])**2).sum()
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # xtr=xtr.reshape(num_train,1,-1)
  # dists=((xtr-xte)**2).sum(dim=2)
  # It works, but slow

  # https://stackoverflow.com/questions/27948363/numpy-broadcast-to-perform-euclidean-distance-vectorized 
  dists = ((xtr**2).reshape(num_train,1,-1).sum(dim = 2) - torch.mm(xtr,xte.T)*2 + (xte**2).sum(dim = 1))
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(indices.shape[1]):
    idx = torch.max(y_train[indices[:,i]].bincount(), dim = 0).indices
    y_pred[i] = idx
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # personally:
  # k_to_accuracies[here are indices] = [...a list...here are accuracies...]
  # the length of the list is num_fords
  # how to get accuracies? classifier.check_accuracy(input test data)
  # who are inputs for classifier? training data
  # ALL BUT ONE FOLD as training data
  # The ONE is the last fold as a validation set
  
  for k in k_choices:
    k_to_accuracies[k] = []
    for i in range(num_folds):
      # xtrain=torch.cat( (torch.cat(x_train_folds[:i]), torch.cat(x_train_folds[i + 1:])) )
      # if i==0 or num_folds, error
      xtrain=torch.cat([x_train_folds[j] for j in range(num_folds) if j != i], dim=0)
      ytrain=torch.cat([y_train_folds[j] for j in range(num_folds) if j != i], dim=0)

      xtest,ytest=x_train_folds[i],y_train_folds[i]
      
      clf = KnnClassifier(xtrain, ytrain)
      k_to_accuracies[k].append(clf.check_accuracy(xtest,  ytest, k,quiet = True))
...
def knn_get_best_k(k_to_accuracies):
  ...
  #   s=sum(k_to_accuracies[i])/len(k_to_accuracies[i])
  #   if  s > maxtemp:
  #     best_k= i 
  #     maxtemp= s
  maxtemp=0.0
